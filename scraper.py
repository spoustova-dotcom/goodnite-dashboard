...
def scrape_reviews_playwright(page, url, cutoff_date):
    ...
    reviews = []
    # Add sort=recent parameter directly in URL - most reliable method
    base_url = url.split("?")[0]
    reviews_url = base_url + "?order=recent#tab-reviews"
    ...
    try:
        ...
        # Also try clicking sort button as backup
        for label in ["Nejnovější", "Most recent", "Newest", "Datum"]:
            try:
                ...
                for sel_pattern in [
                    ...
                    f"[data-value='recent']",
                    f"[data-order='recent']",
                    f"option[value='recent']",
                    f"li[data-id='recent']",
                ]:
                    el = page.query_selector(sel_pattern)
                    if el:
                        el.click()
                        time.sleep(1.5)
                        print(f"    Clicked sort: {sel_pattern}")
                        break
            except:
                pass

        print(f"    Using URL sort: {reviews_url}")
        time.sleep(1)
